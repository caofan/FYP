...
'''
def findNovelJunctions(refDict,inputfile,mismatch,multijunc):
...
if __name__=='__main__':
    if platform.python_version().startswith('2.7'):
        ...
        parser.add_argument('-r','--reference',dest='reference',required=True,help='The reference file should be in bed format or refflat format')
        parser.add_argument('-i','--input',dest='inputfile',required=True,help='The input file that contains the junctions to compare with the reference')
        ...
        findNovelJunctions(refDict,options.inputfile,options.mismatch,options.multijunc)
    else:
